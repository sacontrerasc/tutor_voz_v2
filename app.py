...
# --- Escucha el mensaje postMessage desde Moodle y guarda el correo en session_state ---
components.html("""
<script>
window.addEventListener("message", (event) => {
    ...
    if (correo) {
        ...
        streamlitDoc.postMessage(
            { type: "streamlit:setComponentValue", value: correo },
            "*"
        );
    }
});
...
# --- Guardar el correo recibido en session_state ---
if "email" not in st.session_state:
    st.session_state["email"] = ""

# Recibir el valor actualizado desde la URL (Streamlit lo almacena en query_params)
email = st.query_params.get("email", st.session_state["email"])
...
# --- Responder con IA y contexto Moodle ---
